...
export class Blockchain {
  ...
  minePendingPosts(miningRewardAddress: string): void {
    // In a real implementation, we would select specific posts to include in a block
    // For simplicity, we're mining each pending post as its own block
    const pendingPostsCopy = [...this.pendingPosts]; // Create a copy to avoid modification during iteration
    
    for (const block of pendingPostsCopy) {
      block.previousHash = this.getLatestBlock().hash;
      block.mineBlock(this.difficulty);
      this.chain.push(block);
      console.log(`Post mined and added to the blockchain: ${block.hash}`);
    }
    
    // Clear pending posts after mining
    this.pendingPosts = [];
    
    // In a real blockchain, we would add a mining reward here
    if (miningRewardAddress) {
      this.createPost(`Mining reward transaction`, miningRewardAddress);
    }
  }
  ...
  /**
   * Get all messages for a specific user (sent to or by them)
   */
  ...
  static fromJSON(json: string): Blockchain {
    ...
    // Recreate messages if they exist
    if (data.messages) {
